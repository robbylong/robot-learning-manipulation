...
EXAMPLE_TARGET = "./demos/images/demo_parts/basketball.jpeg"
IMDIR = Path("./demos/images/demo_parts")
...
class HeatmapVisualization(object):
    ...
    def find_best_match(self, event,u,v,flags,param):
        ...
        for m_i, mask_i in enumerate(self.masks):
            mask = torch.stack([torch.Tensor(mask_i).cpu()/255.]*3, dim=-1)
            overlay = torch.ones( img_1_w_annot.shape ).cpu() \
                    * label_colors[m_i].unsqueeze(0).unsqueeze(0).cpu()
            img_1_w_annot = torch.Tensor(img_1_w_annot).cpu()
        ...
        mask = torch.stack([torch.Tensor(self.mask1).cpu()/255.]*3, dim=-1)
        overlay = torch.ones( img_1_w_annot.shape ).cpu() * torch.Tensor([[ [255,0,0] ]]).cpu()
        img_1_w_annot = torch.Tensor(img_1_w_annot).cpu()
        img_1_w_annot = (  img_1_w_annot * (1-mask) \
                      + (img_1_w_annot*0.6+overlay*0.4) * (mask) ) 
        img_1_w_annot = img_1_w_annot.cpu().numpy().astype(np.uint8)
