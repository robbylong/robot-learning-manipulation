...
SUPPORT_DIR = "../affordance_database/temp/"
TARGET_IMAGE_PATH = "./images/rgb_image.jpg"
...
def viz_correspondence(im_a, im_b, parts_a, parts_b):
    ...
    for i, part_i in enumerate(parts_a):
        ...
        mask_coordinates = []
        for row in range(part_out_i.shape[0]):
            for col in range(part_out_i.shape[1]):
                if part_out_i[row][col] == True:
                    mask_coordinates.append([row,col])
