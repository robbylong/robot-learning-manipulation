...
# Cable
PATH_TO_obj = OBJECT_PATH +\
                    "/assets/google/GoPro_HERO3_Composite_Cable/model.sdf"
...
# Create an instance of the Object class to import the object
cable = SDFObject(PATH_TO_obj, object_position, object_orientation)

offset = 0
scale = 1
ball = p.loadURDF("soccerball.urdf",[0.6,0,table_height_offset+0.06], globalScaling=scale*0.1)
p.changeDynamics(ball,-1,linearDamping=0, angularDamping=0, rollingFriction=0.001, spinningFriction=0.001,mass = 2.0)
p.changeVisualShape(ball,-1,rgbaColor=[0.8,0.8,0.8,1])
ball_id = p.getBodyUniqueId(ball)
offset = 2*scale*0.1
# #ball
# PATH_TO_obj = OBJECT_PATH +\
#                     "/assets/google/robocup_3Dsim_ball/model.sdf"
...
# object_orientation = p.getQuaternionFromEuler([1.5708, 0, 0])  # Specify the orientation of the object
...
corr_img = affcor.run_result(rgb,SUPPORT_DIR)
# Save corr_img as an image file
cv2.imwrite("Affcorrs_estimate.jpg", corr_img)
# print(rgb/255)
# print(rgb.shape)
...
for step in range (N_UP_STEPS):
    ...
    #     # Apply the torque to the joint
    #     p.setJointMotorControl2(rpl_panda_w_gripper.id, segment_indices_row[j], p.TORQUE_CONTROL, force=spring_torque/segment_length)
        # p.setJointMotorControl2(
        #     rpl_panda_w_gripper.id,
        #     segment_indices_row[j],
        #     controlMode=p.POSITION_CONTROL,
    ...
    # # # Get the contact points in the simulation
    # contact_points = p.getContactPoints(rpl_panda_w_gripper.id, ball_id, 34)
    # palm_force = 0
    # for contact in contact_points:
    #     print("the contact i is: ")
    #     print(contact)
    #     contact_force = contact[9]
    #     contact_vector = contact[7]
    #     palm_force += contact_force*contact_vector[2]
